...
function FlowNode({ label, x, y, kind = 'source' }) {
  ...
  return (
    <div style={{
      ...
      ...(isRight
        ? { right: '12px', left: 'auto', transform: 'translateY(-50%)' }
        : { left: '12px', right: 'auto', transform: 'translateY(-50%)' }
      ),
      top: `${y}%`,
      padding: '10px 14px',
...
function FlowDiagram() {
  ...
  return (
    <div className="flow-stage" style={{ height: '460px', width: '100%', overflow: 'visible' }}>
      <svg
        ...
        {sources.map((s, i) => (
          <g key={`in${i}`}>
            <path
              ...
              stroke="url(#flowG)" strokeWidth="0.35" fill="none"
      ...
      {/* nodes */}
      {sources.map((s, i) => <FlowNode key={`s${i}`} label={s.label} x={srcX} y={s.y} kind="source" />)}
      <FlowNode label="KRYZON Flow" x={50} y={50} kind="core" />
      {outputs.map((o, i) => <FlowNode key={`o${i}`} label={o.label} x={outX} y={o.y} kind="output" />)}
